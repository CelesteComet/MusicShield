...
import { orangeShieldMaterial } from './materials';
...
class Shield {
  ...
  update() {
    ...
    if (Keyboard.keys['left'])  { mesh.translateOnAxis(new THREE.Vector3(-1,0,0), speed); }
    if (Keyboard.keys['right']) { mesh.translateOnAxis(new THREE.Vector3(1,0,0), speed);  }
    if (Keyboard.keys['up'])    { mesh.translateOnAxis(new THREE.Vector3(0,1,0), speed);  }
    if (Keyboard.keys['down'])  { mesh.translateOnAxis(new THREE.Vector3(0,-1,0), speed); }
  }
}
